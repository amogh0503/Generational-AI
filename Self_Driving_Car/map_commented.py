...
def init():
    ...
    global goal_y
    global first_update# y-coordinate of the goal (where the car has to go, that is the airport or the downtown)
...
class Game(Widget):
    ...
    def serve_car(self): # starting the car when we launch the application
        ...
        self.car.velocity = Vector(4, 0) # the car will start to go horizontally to the right with a speed of 6
    ...
    def update(self, dt): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        if sand[int(self.car.x),int(self.car.y)] > 0: # if the car is on the sand
            ...
            self.car.velocity = Vector(4, 0).rotate(self.car.angle) # it goes to a normal speed (speed = 6)
